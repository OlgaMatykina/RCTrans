...
class Vega:
    ...
    def __init__(self,
                 ...
                 ann_file: str = '/data/sets/nuscenes/info.pkl',
                 verbose: bool = True,
        ...
        self.predictions = self.load_data_dict(annotations)['results']

        self.explorer = VegaExplorer(self)

    
    def load_data_dict(self, annotations):
    ...
    def get_boxes(self, sample_data_token: str) -> List[Box]:
        """
        Возвращает список Box объектов для заданного sample_data_token, основываясь только на предсказаниях.
        """
        ann_records = self.predictions[sample_data_token]  # Предсказания или аннотации
        boxes = []

        for record in ann_records:
            if record['detection_score'] < self.score_thr:
                continue
            if record['detection_name'] not in category_mapping:
                continue
            box = Box(
                record['translation'],
                record['size'],
                Quaternion(record['rotation']),
                name=category_mapping[record['detection_name']],
...
class VegaExplorer:
    ...
    def render_sample(self,
        ...
        camera_data = {'CAM_FRONT': record['image_path'],}
        ...
        cols = 2
        # cols = 3
        fig, axes = plt.subplots(int(np.ceil(n / cols)), cols, figsize=(16, 9))
        ...
        for ax in axes.flatten()[num_radar_plots:]:
            # if show_lidarseg or show_panoptic:
            #     sd_record = self.nusc.get('sample_data', sd_token)
            ...
            #                                     show_panoptic=show_panoptic)
            # else:
                self.render_sample_data('camera', camera_data['CAM_FRONT'], token, box_vis_level=box_vis_level, ax=ax, nsweeps=nsweeps,
    ...
    def _load_points(self, pts_filename):
        ...
        return points
    def load_radar_points(self, path):
    ...
    def render_sample_data(self,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            pc = self.load_radar_points(path)
            ...
            # Show point cloud.
            points = view_points(pc.tensor.T.numpy()[:3, :], viewpoint, normalize=False)
            dists = np.sqrt(np.sum(pc.tensor.T.numpy()[:2, :] ** 2, axis=0))

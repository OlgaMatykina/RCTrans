...
train_pipeline = [
    ...
    dict(
        type='LoadRadarPointsMultiSweepsRadialSpeed',
        load_dim=6,
        sweeps_num=1,
        use_num=6,
...
data = dict(
    ...
    train=dict(
        ...
        box_type_3d='LiDAR'),
    val=dict(type=dataset_type, data_root=data_root, pipeline=test_pipeline, collect_keys=collect_keys + ['img', 'radar', 'img_metas'], queue_length=queue_length, ann_file=ann_root + 'mini_vega_radar_temporal_infos_val.pkl', classes=class_names, modality=input_modality, cam_types=cam_types, radar_types=radar_types),
    test=dict(type=dataset_type, data_root=data_root, pipeline=test_pipeline, collect_keys=collect_keys + ['img', 'radar', 'img_metas'], queue_length=queue_length, ann_file=ann_root + 'mini_vega_radar_temporal_infos_val.pkl', classes=class_names, modality=input_modality, cam_types=cam_types, radar_types=radar_types),
...
# resume_from=None
resume_from='work_dirs/dinov2_with_resnet_from_res18_cam_front/epoch_1.pth'
...
log_config = dict(
    ...
    hooks=[
        ...
        #         name='flashattn dinov2s + adapter + resnet18 RCTrans from res18 CAM_FRONT',     # Имя эксперимента

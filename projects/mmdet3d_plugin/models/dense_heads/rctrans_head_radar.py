...
class RCTransHead_radar(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        super(RCTransHead_radar, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def temporal_alignment(self, tgt, reference_points, img_metas):
        ...
        temp_bevpos = self.query_embed(temp_reference_point, img_metas)
        ...
        return tgt, reference_points, temp_memory, temp_bevpos, rec_ego_pose

    def temporal_alignment_pos(self, query_bevpos, reference_points):
    ...
    def forward(self, memory_center, img_metas, topk_indexes=None,  **data):
        ...
        memory, pos_embed = torch.cat([bev_memory], dim=0).transpose(1,0), torch.cat([bev_pos_embed], dim=0).transpose(1,0)
        #############
        ...
        bev_query_pos = self.query_embed(reference_points, img_metas)
      
        tgt = torch.zeros_like(bev_query_pos)
        ############
        
        # prepare for the tgt and query_pos using mln.
        tgt, reference_points, temp_memory, temp_bevpos, rec_ego_pose = self.temporal_alignment(tgt, reference_points, img_metas)
        ...
        outputs_classes, outputs_coords, outs_dec = self.transformer(memory, tgt, [bev_query_pos], pos_embed, attn_mask, temp_memory, [temp_bevpos], \

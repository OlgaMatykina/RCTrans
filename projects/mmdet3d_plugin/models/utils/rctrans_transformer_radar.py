...
class RCTransTransformerDecoder_radar(TransformerLayerSequence):
    ...
    def __init__(self,
        ...
        super(RCTransTransformerDecoder_radar, self).__init__(*args, **kwargs)
    ...
    def forward(self, query, key, value, key_pos, query_pos, temp_memory, temp_pos, key_padding_mask, attn_masks, \
        ...
        for index in range(int(len(self.layers)/2)):
            ...
            # print('query', query.shape, 'bev_key', bev_key.shape)
            query = self.layers[2*index](query, bev_key, bev_key, bev_query_pos, bev_key_pos, temp_memory, bev_temp_pos, attn_masks) # [Nq, B, C]
            query = self.layers[2*index + 1](query, bev_key, bev_key, bev_query_pos, bev_key_pos, temp_memory, bev_temp_pos, attn_masks) # [Nq, B, C]
            ...
            if index < (int(len(self.layers)/2)-1):
                ...
                bev_query_embeds = query_embed(reference_points, img_metas)
                bev_query_pos = temporal_alignment_pos(bev_query_embeds, reference_points)
